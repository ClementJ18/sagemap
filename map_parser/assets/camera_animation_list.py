...
class FreeCameraAnimationCameraFrame:
    ...
    def parse(cls, context: "ParsingContext"):
        ...
        rotation = (context.stream.readFloat(), context.stream.readFloat(), context.stream.readFloat(), context.stream.readFloat())
        fov = context.stream.readFloat()

        return cls(position=position, rotation=rotation, fov=fov)
...
class LookAtCameraAnimationLookAtFrame:
    ...
    def parse(cls, context: "ParsingContext"):
        ...
        return cls(look_at_point=look_at)
...
class LookAtCameraAnimationCameraFrame:
    ...
    def parse(cls, context: "ParsingContext"):
        ...
        return cls(position=position, roll=roll, fov=fov)
...
class CameraAnimation:
    ...
    @classmethod
    def parse(cls, context: "ParsingContext"):
        animation_type = context.stream.readFourCc()
        ...
        context.logger.debug(f"CameraAnimation: {name}, Type: {animation_type}, Frames: {num_frames}, StartOffset: {start_offset}")
        return cls(animation_type=animation_type, name=name, num_frames=num_frames, start_offset=start_offset, frame_data=frame_data)
